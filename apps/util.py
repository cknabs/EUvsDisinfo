...
def sort(data: DataFrame) -> DataFrame:
    ...
    return (
        ...
        .sort_values(
            by="sum_of_rows", ascending=False, axis="index", kind="mergesort"
        )
        .drop("sum_of_rows", axis="columns")
        .sort_values(
            by="sum_of_cols", ascending=False, axis="columns", kind="mergesort"
        )
...
def plot_sankey(data: DataFrame, min_val: int = 50):
    ...
    fig = go.Figure(
        data=[
            go.Sankey(
                node=dict(label=labels, color="black"),
                link=dict(
                    source=sources, target=targets, value=values, color="gray"
                ),
                orientation="h",
            )
        ]
